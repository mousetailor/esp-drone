...
static void uart_rx_task(void *arg) {
  ...
  while (1) {
    ...
    int got =
        uart_read_bytes(UART_NUM_0, &buf[1], remaining, pdMS_TO_TICKS(50));
    if (got != remaining)
...
static void espnow_tx_task(void *arg) {
  ...
  while (1) {
    if (xQueueReceive(send_queue, buf, portMAX_DELAY) == pdTRUE) {
      esp_err_t ret = esp_now_send(drone_mac, buf, sizeof(control_packet_t));
...
static void uart_init(void) {
  ...
  ESP_ERROR_CHECK(uart_driver_install(UART_NUM_0, 2048, 256, 0, NULL, 0));
}
...
static void wifi_and_espnow_init(void) {
  ...
  ESP_LOGI(TAG, "Target drone MAC: %02X:%02X:%02X:%02X:%02X:%02X", drone_mac[0],
           drone_mac[1], drone_mac[2], drone_mac[3], drone_mac[4],
           drone_mac[5]);
