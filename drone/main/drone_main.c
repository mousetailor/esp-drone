...
static uint8_t controller_mac[6] = {0x88, 0x57, 0x21, 0xAC, 0x69, 0xE8};
...
static float pid_compute(pid_state_t *pid, float setpoint, float measurement,
                         float dt) {
 ...
 *  MOTOR PWM (LEDC)
 * ========================================================= */
static const int motor_gpios[4] = {MOTOR1_GPIO, MOTOR2_GPIO, MOTOR3_GPIO,
                                   MOTOR4_GPIO};
...
static void motors_init(void) {
  ...
  ESP_LOGI(TAG, "Motors initialized on GPIOs %d, %d, %d, %d", MOTOR1_GPIO,
           MOTOR2_GPIO, MOTOR3_GPIO, MOTOR4_GPIO);
}

/* Convert 1000-2000 range to LEDC duty value */
static uint32_t throttle_to_duty(uint16_t throttle_us) {
  if (throttle_us < MOTOR_MIN)
    throttle_us = MOTOR_MIN;
  if (throttle_us > MOTOR_MAX)
    throttle_us = MOTOR_MAX;

  /* Linear interpolation: 1000 -> ESC_DUTY_MIN, 2000 -> ESC_DUTY_MAX */
  return ESC_DUTY_MIN +
         (uint32_t)((throttle_us - MOTOR_MIN) * (ESC_DUTY_MAX - ESC_DUTY_MIN)) /
             (MOTOR_MAX - MOTOR_MIN);
}
...
static bool mpu6050_init(void) {
  ...
  if (ret != ESP_OK || (who != 0x68 && who != 0x72 && who != 0x70)) {
    ESP_LOGE(TAG, "MPU6050 not found (WHO_AM_I=0x%02X, err=%s)", who,
             esp_err_to_name(ret));
    return false;
  }
...
static bool mpu6050_read_raw(int16_t *ax, int16_t *ay, int16_t *az, int16_t *gx,
...
static void on_data_recv(const esp_now_recv_info_t *info, const uint8_t *data,
                         int data_len) {
...
static void imu_task(void *arg) {
  ...
  while (1) {
    ...
    pitch_acc =
        atan2f(-ax_g, sqrtf(ay_g * ay_g + az_g * az_g)) * (180.0f / M_PI);
...
static void telemetry_task(void *arg) {
  ...
  while (1) {
    ...
    tele.checksum = compute_checksum((const uint8_t *)&tele, sizeof(tele) - 1);
    ...
    ESP_LOGD(TAG,
             "Tele: batt=%umV roll=%.1f° pitch=%.1f° yaw=%.1f° "
             "M=[%u,%u,%u,%u] %s%s",
             battery_mv, r, p, y, motor_out[0], motor_out[1], motor_out[2],
             motor_out[3], ctrl_armed ? "ARMED" : "safe",
...
void app_main(void) {
  ...
  ESP_LOGI(TAG, "║  Failsafe timeout: %4d ms            ║",
           FAILSAFE_TIMEOUT_MS);
  ESP_LOGI(TAG, "╚══════════════════════════════════════╝");
}
